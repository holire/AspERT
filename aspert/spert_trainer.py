...
from aspert import models, prediction
from aspert import sampling
from aspert import util
from aspert.entities import Dataset
from aspert.evaluator import Evaluator
from aspert.input_reader import JsonInputReader, BaseInputReader
from aspert.loss import ASpERTLoss, Loss
from tqdm import tqdm
from aspert.trainer import BaseTrainer
# import attention

SCRIPT_PATH = os.path.dirname(os.path.realpath(__file__))


class ASpERTTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: Type[BaseInputReader]):
        ...
        # ASpERT is currently optimized on a single GPU and not thoroughly tested in a multi GPU setup
        # If you still want to train ASpERT on multiple GPUs, uncomment the following lines
        # # parallelize model
        ...
        pre_data_loader = DataLoader(train_dataset, batch_size=self._args.train_batch_size, shuffle=False, drop_last=True,
                                 num_workers=self._args.sampling_processes, collate_fn=sampling.collate_fn_padding)
        attention.get_entity_attention(pre_data_loader)
        '''
    ...
    def _load_model(self, input_reader):
        ...
        config.aspert_version = model_class.VERSION
        model = model_class.from_pretrained(self._args.model_path,
                                            config=config,
                                            # ASpERT model parameters
